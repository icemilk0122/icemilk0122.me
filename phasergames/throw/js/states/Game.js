...
Achicken.GameState = {
  ...
  init: function(currentLevel) {
    ...
    this.game.world.setBounds(0,0,2200,768);
  ...
  create: function() {
    ...
    this.pole = this.add.sprite(180, 628, 'pole');
    this.pole.anchor.setTo(0.5, 0);

    //init chicken shooting
    this.pole2 = this.add.sprite(2000, 628, 'pole');
  ...
  update: function() {
    ...
    if(this.isPreparingShot) {
      ...
      //camera修正
      this.chicken.x = (this.game.input.activePointer.position.x + this.game.camera.x) / this.game.camera.scale.x;
      this.chicken.y = (this.game.input.activePointer.position.y + this.game.camera.y) / this.game.camera.scale.y;

      //if too far away stop shot preparation
      var distance = Phaser.Point.distance(this.chicken.position, curPole.position);

      if(distance > this.MAX_DISTANCE_SHOOT) {
        this.isPreparingShot = false;
        this.isChickenReady = true;

        this.chicken.x = curPole.x;
        this.chicken.y = curPole.y;
      }
  ...
  setupChicken: function() {
    ...
    this.chicken = this.add.sprite(curPole.x, curPole.y, 'chicken');
    this.chicken.anchor.setTo(0.5);

    this.game.camera.follow(this.chicken);
  ...
  refreshStats: function() {
    ...
    while(i < this.numChickens) {
      if(i%2==0)
      {
        this.chickenHUD.create(this.game.width - 100 - i/2 * 80, 30, 'chicken');
      }else {
        this.chickenHUD.create( i * 40, 30, 'chicken');
      }
      i++;
    }
    this.chickenHUD.fixedToCamera = true;
  }
