...
$(function(){
	...
	$.ajax({
		...
		success:function(response){
			...
			for(var i=0;i<len;i++)
			{
				adarr.push({img:response.ad[0].slider[i]["path"+lang],url:response.ad[0].slider[i]["link"+lang]});
			}
			for(var j=0;j<adarr.length/2;j++)
			{
				ad2arr.push({img1:adarr[2*j].img,url1:adarr[2*j].url,img2:adarr[2*j+1].img,url2:adarr[2*j+1].url});
			}
			init();
		},
		beforeSend:function(){
